...
import { fetchPedidoDelete } from './fetch-ms-pedidos';
import { usePathname, useSearchParams, useRouter } from 'next/navigation';

export function ActualziarPedido({ id }: { id: string }) {
    return (
      <Link
        href={`/dashboard/pedidos/editar/${id}`}
...
export function EliminarPedido({ id }: { id: string }) {
    ...
    const handleConfirm = async () => {
        ...
        const result = await fetchPedidoDelete(basePath, id);
        ...
        if (result) {
            ...
            alert("Error al eliminar al pedido");
        }
            ...
            {isDialogOpen && (
                ...
                <div className="modal-box">
                    ...
                    <div className="modal-action flex justify-between w-full">
                        <button 
                            disabled={isLoading}
                            className="btn bg-red-800 rounded px-2 py-1 hover:bg-blue-700 hover:text-white" 
                            onClick={closeDialog}
                            >
                            Cancelar
                        </button>
